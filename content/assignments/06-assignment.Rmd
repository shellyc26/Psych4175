...
This assignment (and the three following assignments) are about using `R` to reinforce the statistical principles you learned about in Psych Stats. You may use any of the datasets posted on Canvas or you may use an outside dataset (must be approved by Shelly). **I STRONGLY SUGGEST USING THE SAME DATASET THAT YOU USED ON ASSIGNMENT 1**
...
**Hint:** Check out `geom_bar` instead of `geom_histogram`. Not sure how to make every number between 0 and 20 show up on your x-axis? You'll need to Google for how to change the scale of the x-axis.
