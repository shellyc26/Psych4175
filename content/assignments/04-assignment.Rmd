...
Create a `.Rmd` file (either RNotebook or RMarkdown is fine). When you turn in the assignment, I want you to submit ***BOTH*** the `.html` file ***and*** the `.Rmd` file. Please do NOT submit the `.nb.html` file, however. You are free to choose the topics -- it can be anything we've covered!
...
Here's your to-do list (e.g., your file must include the following):
  ...
  - There must be (at least) 3 subheaders. They must be notably smaller than the larger headers, indicated that these are subsections of the topic you're covering. You do NOT need 3 subheaders for each of your 4 topics. Just at total of 3 subheaders.
...
### So, so wonderful!
Submit your `.Rmd` and `.html` file to Canvas (but not the `.nb.html` file).
