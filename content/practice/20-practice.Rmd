...
<img src="../encouragementFigs/figure-html/spongememe.jpg" width = "100%">
...
blogdown::shortcode("learnr",
                    url = "https://shelly-cooper.shinyapps.io/20-learnr-quiz1/",
                    id = "learnr-20-quiz1")
...
blogdown::shortcode("learnr",
                    url = "https://shelly-cooper.shinyapps.io/20-learnr-quiz2/",
                    id = "learnr-20-quiz2")
...
blogdown::shortcode("learnr",
                    url = "https://shelly-cooper.shinyapps.io/20-learnr-app1/",
                    id = "learnr-20-app1")
...
<img src="../encouragementFigs/figure-html/rockememe.jpg" width = "50%">
...
:::fyi
**FYI:** R has a built in function called `ifelse()`. It does the same thing as an `if/else` statement. However, it only has 2 conditions. That it, it does not have the opportunity to use `else if` for multiple conditional statements. 
...
Let's expand our provel example. Write an `if/else if/else` statement. Say that 1-3 corresponds to "dislike", 4-6 corresponds to "neutral", and anything above a score of 7 corresponds to "like".

```{r, echo=FALSE}

blogdown::shortcode("learnr",
                    url = "https://shelly-cooper.shinyapps.io/20-learnr-app2/",
                    id = "learnr-20-app2")
...
For this example, I have generated a *population* of data. The true population mean of this dataset is `r mean(pop_data)`.

Here is a histogram of the population data:

```{r Summarize population data}
ggplot(pop_data, aes(x = pop_data)) +
...
Now, we can use a for loop to draw random samples *with replacement* from the population. For this first attempt, we will draw a small number of samples (10) to create `sample_means_small`. 
...
Next, we will draw a slightly larger number of samples (100) to create `sample_means_medium`. 
...
plot1 <- ggplot() +
  geom_histogram(data = sample_means_small, aes(values), fill = "plum4", color = "black", binwidth = 5) +
  theme_classic() +
  xlab("Sample means") +
  ylab("Count") +
  geom_vline(xintercept = small_m, linetype = "longdash") +
  geom_vline(xintercept = pop_m) +
  coord_cartesian(xlim = c(85, 115)) +
  ggtitle("Distribution of 10 samples")

plot2 <- ggplot() +
  geom_histogram(data = sample_means_small, aes(values), fill = "plum4", color = "black", binwidth = 3) +
  theme_classic() +
  xlab("Sample means") +
  ylab("Count") +
  geom_vline(xintercept = medium_m, linetype = "longdash") +
  geom_vline(xintercept = pop_m) +
  coord_cartesian(xlim = c(85, 115)) +
...
plot3 <- ggplot() +
  ...
  xlab("Sample means") +
  ylab("Count") +
  geom_vline(xintercept = large_m, linetype = "longdash") +
  geom_vline(xintercept = pop_m) +
  coord_cartesian(xlim = c(85, 115)) +
...
blogdown::shortcode("learnr",
                    url = "https://shelly-cooper.shinyapps.io/20-learnr-quiz3/",
                    id = "learnr-20-quiz3")

```

:::fyi
**Reminder:** The 95% confidence interval is calculated using the following formula:

$CI_{95} = \bar{X} \pm (1.96 * SEM)$

:::

Now we're going to use *bootstrapping* to calculate the 95% confidence intervals.

**Tips and Instructions:**
* Use `pop.data` for the means!
* See the Power Plus lecture slides 10-11 for examples of calculating a single CI
* Use z = 1.96 for a 95% interval
* On each loop iteration, save the upper CI to `upper_values`, and the lower CI to `lower_values` (see `append()` help page)

```{r, echo=FALSE}

blogdown::shortcode("learnr",
                    url = "https://shelly-cooper.shinyapps.io/20-learnr-app3/",
                    id = "learnr-20-app3")
