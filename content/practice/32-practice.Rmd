...
kable(china_sum_1[c(1:10), c(73:77)])
```
...
kable(china_sum_2[c(1:10), c(73:77)])
```
...
# LOOK AT TIMING AND CUT SECTION IF NEEDED
...
(summ_Open <- summary(fit_Open)) #if you put parentheses around the whole expression it will print it out below
```
...
For some help, the mean is not explicitly in the data frame, so you will either have to create an Openness mean score for females and males in the data frame.
