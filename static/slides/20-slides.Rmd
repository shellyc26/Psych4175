...
The next 5 weeks are going to be dedicated in getting you up to speed on statistics. Everyone in this class should have taken Psych 300 (Intro Psych Stats) or equivalent. **_If you have not taken either of these classes, you must get in touch with me ASAP._**
...
- There will be an accompanying Practice Set for each lecture (hopefully...I'm trying!). In these, we will cover functions that are particularly useful when applying this theoretical knowledge.
...
- Descriptive Stats (moments of a distribution)
- Bias
- $z$-scores
...
- Deviations from the mean sum to 0
  ...
  - Sum of Deviation Scores: $\Sigma(x - \bar{x})$
  - $\Sigma(x - \bar{x}) = 0$

--

- Can only be used with interval- and ratio-level variables
  - no nominal or ordinal variables
...
- **Median** -- the middle point of the data
  ...
  - Must put them in order first (highest to lowest or vice versa), then find the middle point
    - If even number, get the "middle" of the 2 middle-est numbers (average of 2 middle-est)
...
**Kurtosis** = pointyness
  - Too pointy = leptokurtic
  - Perfect = mesokurtic
  - Too flat = platykurtic (as in platypus!)
...
# Why not use raw scores for everything?
```{r, echo=FALSE}
