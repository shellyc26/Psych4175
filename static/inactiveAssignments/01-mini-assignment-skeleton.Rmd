...
## How this works
This mini assignment is on the topic of `for loops`. There are 4 problems, each ramping up in terms of difficulty and points:
...
HINT: I would suggest looking at the `bfi` dataset from inside your RStudio. You should not print it out in your code. But take a look at the dataset before you manipulate the broken code below.

HINT 2: The only package you need is the `psych` package for accessing the dataset. The goal is to practice for loops. Not other functions. 

HINT 3: Start by running the code. Address any error messages that may or may not come up. It is your job to check the output and make sure it is doing what it's supposed to. Think critically, here!
...
We are going to play a game. It's called Fizz Buzz. You are going to print integers 1 to 50, but print “Fizz” if an integer is divisible by 3, “Buzz” if an integer is divisible by 5, and “FizzBuzz” if an integer is divisible by both 3 and 5.

To do this, let's introduce you to the **modulus operator**. We use 2 percent signs, like this `%%`. It returns the *remainder* of a division. So for instance `4 %% 2` will result in `0`. That's because 2 goes into 4. There is no remainder after that division! If you instead did `17 %% 7` the result would be `3`. That's because 7 goes into 17 two times, but there are 3 left over. This is often used to figure out if something is an even number or not. 

You're going to make this one from scratch. Think carefully about the logic you want! I have provided you with the 100 numbers, and the empty vector called `game`. At the end, we're going to print your vector. 

HINT: The key to Fizz Buzz is in the Practice Set for `for loops`. Go to the section that helps you recode provel cheese ratings!

```{r}
fizzbuzz_numbers <- 1:100
...
We have talked about nested functions. Arguably, even more important though is nested `for loops`. I have used this in my own work quite a bit! Something like "for each cognitive task performed at each time point, do something". 
