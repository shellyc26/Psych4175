...
## How this works
This mini assignment is on the topic of *writing your own functions*. There are 4 problems, each ramping up in terms of difficulty and points:
...
This class is designed a little too easily where each assignment is just worth that amount of points and there are 100 points total. So we're not going to use this class (because seriously, it's just counting up raw points). We'll use a hypothetical class. In our hypothetical class:
...
- Think about how you're going to add in your add in your final exam grade and how that aligns with your weights
...
- Add text to the plot itself that tells you the correlation between the 2 variables. Put it as close to the top right corner as is feasible (meaning, I don't want you specifying specific values per plot -- remember, this is supposed to be generalizable).
...
3. `carat` vs. `price` of the `diamonds` data.frame
4. `brainwt` vs. `bodywt` of the `msleep` data.frame
5. `listings` vs. `sales` of the `txhousing` data.frame

Note that items 1 & 2 are built into base R. Items 3-5 are all data.frames that are part of the `ggplot2` package, so you can take a look at them if you're interested. 
